...
export default function Paypal({ json, money, paymentType }) {
  ...
  const urlEditCentro = "https://nailing-sprint3.herokuapp.com/centros/edit"
          ...
          switch (paymentType) {
            case "Reserve":
              $.ajax({
                ...
                data: json,
                url: "https://nailing-sprint3.herokuapp.com/cita/add",
              ...
              break;
            case "NewCentre":
              const urlCentre = "https://nailing-sprint3.herokuapp.com/centros/add/" + user.id.toString();
              postData(urlCentre, json, {
                "Content-Type": "application/json",
                "Authorization": "Basic " + btoa(user.usuario + ":" + user.contrasenya)
              }).then(async function () {
                const url = "https://nailing-sprint3.herokuapp.com/login";
                const body = {
                  "user": user.usuario,
