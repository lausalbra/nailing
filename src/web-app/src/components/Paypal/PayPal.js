...
export default function Paypal({ json, money, paymentType }) {
  ...
  const urlEditCentro = "https://nailing-sprint6.herokuapp.com/centros/edit"
  const urlLogout = "https://nailing-sprint6.herokuapp.com/logout"
  const headers = {
          ...
          switch (paymentType) {
            case "Reserve":
              $.ajax({
                ...
                data: json,
                url: "https://nailing-sprint6.herokuapp.com/cita/add",
              ...
              break;
            case "NewCentre":
              const urlCentre = "https://nailing-sprint6.herokuapp.com/centros/add/" + user.id.toString();
              postData(urlCentre, json, {
                "Content-Type": "application/json",
                "Authorization": "Basic " + btoa(user.usuario + ":" + user.contrasenya)
              }).then(async function () {
                const url = "https://nailing-sprint6.herokuapp.com/login";
                const body = {
                  "user": user.usuario,
