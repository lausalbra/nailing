    ...
    if (state.id !== "" && state.buttons.length === 0) {
      if (user == null) {
        ...
        $.ajax({
          ...
          url: "https://nailing-sprint5.herokuapp.com/tipos/centro/" + state.id.toString(),
...
async function valorar(valoracion){
  ...
  const url= "https://nailing-sprint5.herokuapp.com/valoraciones/add/centro"
...
if(user!=null && !haValorado && primera){
  $.ajax({
    ...
    url: "https://nailing-sprint5.herokuapp.com/valoraciones/list",
    success: function (data) {
      console.log("Servicios recibidos");
...
async function editarValoracion(valoracion, antigua){
  ...
  const url= "https://nailing-sprint5.herokuapp.com/valoraciones/edit"
